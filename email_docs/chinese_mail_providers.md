...
- **Native inbound parse:** **M365 by 21Vianet** (via Microsoft Graph Change Notifications) and **SendCloud** (via built-in Routing/Inbound Parse webhook). Every other provider needs an inbound MX split to a third party (Mailgun/SendGrid/Postmark) or a self-hosted Python IMAP/aiosmtpd listener.
