...
> 5. AuroraSendCloud (global product of SendCloud, by Sohu)
...
### 4.5 AuroraSendCloud (global product of SendCloud, by Sohu)
...
- **Easiest for an Indian developer to sign up & test with Python today:** **Alibaba Cloud DirectMail** and **Tencent Cloud SES** (both fully open with Indian details; Alibaba Enterprise Mail also works but with mailbox constraints). **AuroraSendCloud** (the global product line of SendCloud) also worked from India in this project's own implementation, with no +86-mobile signup gate documented for it — that gate applies to the separate domestic `sendcloud.net` console (see §4.5).
- **Best free entry point:** **Alibaba Cloud DirectMail** (2,000 emails/day free tier; PAYG at $0.29/1,000 emails after that) and **Tencent Cloud SES** (1,000 free emails one-time allowance). **AuroraSendCloud** has no confirmed free tier — it is prepaid credit packages only, starting at $29.90 for 10,000 emails (see §4.5); the 10 emails/day sandbox is documented for the domestic `sendcloud.net` product, not confirmed for AuroraSendCloud. *(Alibaba **Enterprise** Mail has no free tier — it is a paid per-seat product from ~$2.87/user/month, 3-seat minimum; use DirectMail for Alibaba's free quota.)*
...
- **Registrable but heavily gated — require a +86 China mobile:** **NetEase Enterprise Mail (NetEase QiYe)** (passport-based verification works, so no Chinese business license strictly required — but officially not recommended for overseas/global developer use). The domestic **sendcloud.net** console strictly requires +86 for signup OTP with no documented workaround — but this gate is not confirmed for **AuroraSendCloud**, the global product line actually used by this project (see §4.5).
- **Cannot be self-served with Indian details (need Chinese identity/business):** **M365 by 21Vianet**.
- **Support truly dynamic "From" addresses (domain-level auth):** **MXtoChina, Tencent Cloud SES, AuroraSendCloud** — YES. All others require pre-provisioned senders.
- **Native inbound parse:** **AuroraSendCloud** (via its built-in Inbound Route webhook) — though it is reply-tracking only, needs a catch-all address pattern for dynamic senders, and never relays attachments (see §4.5). **M365 by 21Vianet** has no native inbound parse — requires **Power Automate (21Vianet)** for webhook-style routing or Microsoft Graph Change Notifications for event-driven MIME fetch (OAuth via chinacloudapi.cn). Every other provider needs an inbound MX split to a third party (Mailgun/SendGrid/Postmark) or a self-hosted Python IMAP/aiosmtpd listener.
